			...
			>
				<div class="pt-20">
					...
					<div class="flex items-center justify-center">
						<NuxtLink
							to="/"
							class="bg-[#FD374F] mt-4 text-white font-bold py-2 px-4 rounded opacity-50 cursor-pointer"
						>
							Buy Products
						</NuxtLink>
					</div>
			...
			<div v-else class="md:flex gap-4 justify-between mx-auto w-full">
				...
				<div class="md:w-[35%]">
					<div id="Summary" class="bg-white rounded-lg p-4">
						...
						<AlertMsg v-if="alertMsg"
							>Please select an item(s) to checkout</AlertMsg
						>
					</div>
...
<script setup>
	...
	const goToCheckout = () => {
		...
		if (selectedArray.value.length < 1) {
			alertMsg.value = true;
			setTimeout(() => (alertMsg.value = false), 2000);
		} else {
